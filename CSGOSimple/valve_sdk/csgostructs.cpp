...
bool C_BaseCombatWeapon::IsKnife(CCSWeaponInfo* data)
{
	return data->iWeaponType == WEAPONTYPE_KNIFE;
}

bool C_BaseCombatWeapon::IsRifle(CCSWeaponInfo* data)
{
	switch (data->iWeaponType)
	{
...
bool C_BaseCombatWeapon::IsPistol(CCSWeaponInfo* data)
{
	switch (data->iWeaponType)
	{
...
bool C_BaseCombatWeapon::IsSniper(CCSWeaponInfo* data)
{
	switch (data->iWeaponType)
	{
...
void C_BasePlayer::SetAngle2(QAngle wantedang)
{
	...
	static SetAngleFn SetAngle2 = reinterpret_cast<SetAngleFn>(Utils::PatternScan(GetModuleHandle( L"client_panorama.dll" ), "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1"));
	SetAngle2(this, wantedang);
}

void C_BasePlayer::InvalidateBoneCache()
{
	static auto InvalidateBoneCacheFn = Utils::PatternScan(GetModuleHandle( L"client_panorama.dll" ), "80 3D ?? ?? ?? ?? ?? 74 16 A1 ?? ?? ?? ?? 48 C7 81");
...
float C_BasePlayer::GetFlashBangTime()
{
	...
	static uint32_t m_flFlashBangTime = *(uint32_t*)((uint32_t)Utils::PatternScan(GetModuleHandle( L"client_panorama.dll" ), "F3 0F 10 86 ?? ?? ?? ?? 0F 2F 40 10 76 30") + 4);
...
{
	static auto inReload = *(uint32_t*)(Utils::PatternScan(GetModuleHandleW( 
		L"client_panorama.dll" ), "81 C7 ? ? ? ? 8B 4F 0C 8B 57 04 89 4C 24 0C") + 2);
	return *(CUtlVector<IRefCounted*>*)((uintptr_t)this + inReload);
}

...
void C_GrenadePrediction::Tick(int buttons)
{
...
void C_GrenadePrediction::View(CViewSetup* setup)
{
	if (!g_Options.esp_grenadeprediction)
		return;
...
void C_GrenadePrediction::Paint()
{
	if (!g_Options.esp_grenadeprediction)
		return;
	if ((type) && path.size()>1)
	{
		...
		for (auto it = path.begin(), end = path.end(); it != end; ++it)
		{
			...
			{
				g_VGuiSurface->DrawSetColor(g_Options.color_grenadeprediction);
...
void angle_vectors2(const QAngle &angles, Vector *forward, Vector *right, Vector *up)
{
...
void C_GrenadePrediction::Setup(Vector& vecSrc, Vector& vecThrow, QAngle viewangles)
{
	if (!g_Options.esp_grenadeprediction)
		return;
...
void C_GrenadePrediction::Simulate(CViewSetup* setup)
{
	if (!g_Options.esp_grenadeprediction)
		return;
...
int C_GrenadePrediction::Step(Vector& vecSrc, Vector& vecThrow, int tick, float interval)
{
...
bool C_GrenadePrediction::CheckDetonate(const Vector& vecThrow, const trace_t& tr, int tick, float interval)
{
	switch (type)
	{
	case WEAPON_SMOKE:
...
void C_GrenadePrediction::TraceHull(Vector& src, Vector& end, trace_t& tr)
{
	if (!g_Options.esp_grenadeprediction)
		return;
	Ray_t ray;
...
void C_GrenadePrediction::AddGravityMove(Vector& move, Vector& vel, float frametime, bool onground)
{
	if (!g_Options.esp_grenadeprediction)
		return;
...
void C_GrenadePrediction::PushEntity(Vector& src, const Vector& move, trace_t& tr)
{
	if (!g_Options.esp_grenadeprediction)
		return;
...
void C_GrenadePrediction::ResolveFlyCollisionCustom(trace_t& tr, Vector& vecVelocity, float interval)
{
	if (!g_Options.esp_grenadeprediction)
		return;
...
int C_GrenadePrediction::PhysicsClipVelocity(const Vector& in, const Vector& normal, Vector& out, float overbounce)
{
